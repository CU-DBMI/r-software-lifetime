...
combined <- 
  ...
  summarize(
    ...
    close_date = max(close_date, na.rm = TRUE),
    is_vulnerable = any(is_vulnerable), 
    nickname = nickname[which.min(date)],
    .groups = 'drop'
    )
```
...
Current and historical vulnerability tracking information for R. This should not be considered a recommendation for which versions of R to use, but rather a reference resource to help users stay informed and make their own decisions based on official guidance and project documentation.
